...
import {
  TREE_ID_INSERT_ADDR,
  TREE_ITEM_ALL_PACKAGES,
  TREE_ITEM_ID_RECENT_PACKAGES_EMPTY,
  TREE_ITEM_PACKAGE,
  TREE_ITEM_RECENT_PACKAGES,
} from "../common/Consts";
...
const empty_tree: TreeViewBaseItem[] = [
  {
    ...
    children: [{ id: TREE_ITEM_ID_RECENT_PACKAGES_EMPTY, label: "" }],
  },
];
...
function updateViewExplorerState(
  state: ViewExplorerState,
  workdir_json: SuibaseJsonWorkdirPackages
): ViewExplorerState {
  ...
  for (let uuid in moveConfigs) {
    ...
    if (config.packages != null) {
      const folderPathRecent = `${TREE_ITEM_RECENT_PACKAGES}-${uuid}`;
      const folderPathAll = `${TREE_ITEM_ALL_PACKAGES}-${uuid}`;
      for (let package_json of config.packages) {
        if (most_recent) {
          most_recent = false;
          let recentPackageItem = buildTreeViewPackage(folderPathRecent, package_json);
          if (recentPackageItem) recent_packages_folder.push(recentPackageItem);
        }
        const allPackageItem = buildTreeViewPackage(folderPathAll, package_json);
        if (allPackageItem) all_packages_folder.push(allPackageItem);
      }
    }
  }
  ...
  newState.tree.push({
    id: TREE_ITEM_RECENT_PACKAGES,
    label: "Recent Packages",
    children: recent_packages_folder,
  });
...
function buildTreeViewPackage(folderPath: string, json: any): TreeViewBaseItem | undefined {
  // json example: {"pid":"f774c6336e5e0cbb6175dd14f1cc96c4bdde15639e6e1fd876de0b54ef97de88",
  //                "name":"demo",
  //                "ts":"1715418730452",
  //                "init":null
  //               }
  ...
  if (!packageName) {
    console.log("Missing name field in package json: " + JSON.stringify(json));
    return undefined;
  }
  const packageId = json.pid;
  if (!packageId) {
    console.log("Missing pid field in package json: " + JSON.stringify(json));
    return undefined;
  }
...
export function ExplorerTreeView({
  ...
  const [explorerState, setExplorerState] = useState<ViewExplorerState>(
    new ViewExplorerState(workdir, workdirIdx)
  );
