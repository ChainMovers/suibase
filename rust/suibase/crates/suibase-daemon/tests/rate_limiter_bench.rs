...
fn bench_rate_precision() {
    ...
    for rate in rates_to_test {
        ...
        assert!(
            rate_accuracy >= 85.0 && rate_accuracy <= 115.0,
            "Rate accuracy {:.1}% is outside acceptable range",
            rate_accuracy
        );
    }
}
...
fn bench_memory_usage() {
    ...
    println!(
        "Created {} rate limiters in {:?}",
        num_limiters, creation_time
    );
    println!(
        "Memory per limiter: ~{} bytes",
...
fn bench_mixed_workload() {
    ...
    for (name, rate) in &server_configs {
        limiters.push((
            name.to_string(),
            Arc::new(RateLimiter::new(*rate, 0).unwrap()),
        ));
    }
