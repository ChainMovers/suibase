...
Prior to Sui 0.28.x, to submit transactions with multiple commands, one was required to invoke the recently renamed `unsafe_batchTransaction`. This is somewhat limited to allowing only calls to
...
- Not all commands return a re-usable result. For example: `transfer` does not return a reusable result
...
- Commands can only operate with multiple objects for which the primary sender can sign. In other words, if one command is
  ...
  way to include another signer. This restriction and the associated signing limitation are reported to be in review to hopefully ease this constraint
