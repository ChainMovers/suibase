...
- **Unit (pure logic, no live localnet):** `cargo test --features localnet --lib`
  (blob-id parse, descriptor null-normalization, direct-rpc wallet rewrite, fs key).
...
- **CI:**
  ...
  - `walrus-localnet-integration.yml` — heavy on-demand/weekly: builds the mock + deploy
    bin, deploys nodeless Walrus on a real localnet via the regen hook, runs unit + all
    three live suites.

```bash
# Local run (needs: localnet started, walrus_enabled, regen'd so the deploy ran)
...
- Crate: `rust/walrus-store/` (mock `src/localnet.rs`, API `src/lib.rs`, deploy bin
